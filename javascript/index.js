...
function updateDOMWithProfileData(data) {
  ...
  let respositoryCountBadges =
    document.getElementsByClassName("repositories-count");
  ...
  let statusEmojiHolder = document.getElementsByClassName(
    "user-status-emoji-holder"
  );
  ...
  let starredRepositoriesCount = document.getElementById(
  ...
  for (let i = 0; i < contactTwitter.length; i++) {
    if (contactTwitter[i].tagName !== undefined) {
      ...
      a.setAttribute(
        "href",
        "https://twitter.com/" + data.user.twitterUsername
      );
...
function getProfileData() {
  ...
  let token = "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";
  let headers = new Headers({
    Authorization: "Bearer " + token,
    "Content-type": "application/json",
  });
  let body =
    '{ "query": "query { user(login:\\"ifeoluwa00\\") { name url login bio avatarUrl followers { totalCount } following { totalCount } location email twitterUsername websiteUrl starredRepositories { totalCount } status { id emoji emojiHTML message} repositories(first: 20, orderBy: {field:UPDATED_AT, direction:DESC}) { totalCount nodes { updatedAt licenseInfo { name } viewerHasStarred description name url isPrivate forkCount stargazerCount primaryLanguage { id name color } owner { login } defaultBranchRef { name } } } } }" }';
